...
import { PurchaseTransaction, TallyConfig } from '../types';
import { fetchPurchases } from '../services/tallyService';
...
const PurchaseRegister: React.FC<PurchaseRegisterProps> = ({ config, companyId }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('All');
  const [selectedTransaction, setSelectedTransaction] = useState<PurchaseTransaction | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<{data: PurchaseTransaction[], total: number}>({ data: [], total: 0 });
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const result = await fetchPurchases(config, companyId, currentPage, PAGE_SIZE);
        setData(result);
      } catch (e) {
  ...
  // Client-side filtering for demo (Ideally filtering happens on server/API)
  ...
  const handleExport = () => {
    ...
    // Define CSV headers
    const headers = ['Date', 'Invoice No', 'Supplier Name', 'Amount', 'Status'];
    ...
    link.href = url;
    link.setAttribute('download', `Purchase_Register_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Purchase Register</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage and view all purchase vouchers</p>
        </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-100 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold text-slate-600 dark:text-slate-300">Supplier Name</th>
